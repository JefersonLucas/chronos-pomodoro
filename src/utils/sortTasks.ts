 ...
 *
 * O método [.sort()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/sort) recebe uma função que compara dois itens (a, b) e deve retornar:
 ...
 *
 *
 * @param field Qual campo da tarefa será usado para ordenar (opcional)
 * @param direction Se a direção não for informada, usamos `'desc'` (decrescente)
 * @param tasks Lista de tarefas que será ordenada
 * @returns {TaskModel[]} a lista de tarefas ordenada.
 */
export function sortTasks({
	field = "startDate", // Se o campo não for informado, usamos 'startDate' como padrão
	direction = "desc", // Direção da ordenação: crescente ou decrescente (opcional)
